...
export class TokensTradedEventService {
    ...
    async get(params: TokensTradedEventQueryParams = {}): Promise<TokensTradedEvent[]> {
        const { startBlock, endBlock, startTime, endTime, limit, type, pairId, last24h, order } =
            params;
        ...
        if (startTime !== undefined) {
            queryBuilder.andWhere('tokensTradedEvents.timestamp >= :startTime', {
                startTime: new Date(startTime * 1000),
            });
        }

        if (endTime !== undefined) {
            queryBuilder.andWhere('tokensTradedEvents.timestamp <= :endTime', {
        ...
        if (params.normalizeDecimals === true) {
            trades.forEach((t) => {
                t.sourceAmount = new Decimal(t.sourceAmount)
                    .div(`1e${t.sourceToken.decimals}`)
                    .toString();
                t.targetAmount = new Decimal(t.targetAmount)
                    .div(`1e${t.targetToken.decimals}`)
                    .toString();
                if (t.byTargetAmount) {
                    t.tradingFeeAmount = new Decimal(t.tradingFeeAmount)
                        .div(`1e${t.sourceToken.decimals}`)
                        .toString();
                } else {
                    t.tradingFeeAmount = new Decimal(t.tradingFeeAmount)
