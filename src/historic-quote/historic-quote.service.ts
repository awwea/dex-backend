...
export class HistoricQuoteService implements OnModuleInit {
    ...
    async seed(): Promise<void> {
        ...
        for (let startIndex = 0; startIndex < tokens.length; startIndex += batchSize) {
            ...
            const quotesByAddress = await this.coinmarketcapService.getHistoricalQuotes(
                addresses,
                start,
                end
            );
            ...
            for (const token of batchTokens) {
                ...
                await Promise.all(batches.map((batch) => this.repository.save(batch)));
                console.log(
                    `History quote seeding, finished ${++i} of ${tokens.length}%`,
                    new Date()
                );
            }
        }
    ...
    async getHistoryQuotes(
        addresses: string[],
        start: number,
        end: number
    ): Promise<{ [key: string]: HistoricQuote[] }> {
        try {
            ...
            await Promise.all(
                addresses.map(async (tokenAddress) => {
                    const quotes = await this.repository
                        ...
                        .andWhere(
                            'hq.timestamp BETWEEN TO_TIMESTAMP(:start) AND TO_TIMESTAMP(:end)',
                            { start, end }
                        )
            ...
            console.error(
                `Error fetching historical quotes for addresses between ${start} and ${end}:`,
                error
            );
    ...
    async getHistoryQuotesBuckets(
      ...
      ORDER BY
        ...
        if (maxNonNullOpenIndex === -1) {
            throw new BadRequestException({
                message: [
                    'No data available for the specified token addresses. Try a more recent date range',
                ],
                error: 'Bad Request',
                statusCode: 400,
            });
