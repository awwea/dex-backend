        ...
        try {
            ...
            const tokenIds = tokenAddresses.map((address) => {
                ...
                const foundToken = data.find(
                    (token) => token.platform?.token_address.toLowerCase() === address.toLowerCase()
                );
    ...
    private async getV1CryptocurrencyMapTokens(): Promise<any[]> {
        ...
        try {
            ...
            while (true) {
                ...
                const ethereumTokens = responseData.filter(
                    (token) => token.platform && token.platform.slug === 'ethereum'
                );
            ...
            Object.keys(data).forEach((key) => {
                ...
                const tokenAddress =
                    q.id === ETH_ID
                        ...
                        : q.platform.token_address.toLowerCase();
                result.push({
                    tokenAddress,
    ...
    async getHistoricalQuotes(
        ...
        try {
            ...
            const totalDataPoints = Math.ceil(
                ((end - start) / (INTERVAL_IN_MINUTES * 60)) * tokenAddresses.length
            );
            ...
            for (let i = 0; i < batches; i++) {
                ...
                const intervalEnd = moment
                    .unix(Math.min(start + (i + 1) * intervalInSeconds, end))
                    .toISOString(true);
